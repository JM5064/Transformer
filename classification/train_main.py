import torch.nn as nn
import torch.optim as optim
from torch.utils.data.dataloader import DataLoader

from model.bumblebee.bumblebee import Bumblebee
from train import train
from model.loss import CrossEntropyLoss
from data.uai_data.uai_dataset import UAIDataset
from utils import DEVICE, load_checkpoint


def freeze_model(model):
    for param in model.parameters():
        param.requires_grad = False


if __name__ == "__main__":
    # Load dataset
    vocab_json = 'data/wikitext103/vocab.json'
    uai_train = UAIDataset('data/uai_data/uai_dataset/train')
    uai_val = UAIDataset('data/uai_data/uai_dataset/val')
    uai_test = UAIDataset('data/uai_data/uai_dataset/test')

    # Define params
    VOCAB_SIZE = 30000
    SEQ_LEN = 128
    BATCH_SIZE = 32
    NUM_CLASSES = 10
    PAD_INDEX = 1

    # Create dataloaders
    train_loader = DataLoader(uai_train, batch_size=BATCH_SIZE, shuffle=True, num_workers=2)
    val_loader = DataLoader(uai_val, batch_size=BATCH_SIZE, shuffle=False, num_workers=2)
    test_loader = DataLoader(uai_test, batch_size=BATCH_SIZE, shuffle=False, num_workers=2)


    d_model = 512
    model = Bumblebee(vocab_size=VOCAB_SIZE, d_model=d_model)
    model, _, _, _ = load_checkpoint('runs/wikitext103/best.pt', model)

    # Freeze model
    freeze_model(model)

    # Replace last layer
    model.unembedding = nn.Sequential(
        nn.Flatten(),
        nn.Linear(in_features=d_model*SEQ_LEN, out_features=NUM_CLASSES)
    )
        
    loss_func = CrossEntropyLoss(label_smoothing=0.1, pad_index=PAD_INDEX)
    optimizer = optim.Adam(model.parameters(), lr=0.0001, betas=(0.9, 0.98), eps=1e-9, weight_decay=0)

    model = model.to(DEVICE)

    num_epochs = 4
    total_steps = len(train_loader) * num_epochs
    num_warmup_steps = total_steps // 10


    def get_scheduler(optimizer, num_warmup_steps, total_steps):
        warmup_scheduler = optim.lr_scheduler.LinearLR(optimizer, start_factor=1/num_warmup_steps, total_iters=num_warmup_steps)

        cosine_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=total_steps-num_warmup_steps, eta_min=1e-5)

        return optim.lr_scheduler.SequentialLR(optimizer, schedulers=[warmup_scheduler, cosine_scheduler], milestones=[num_warmup_steps])
    
    scheduler = get_scheduler(optimizer, num_warmup_steps, total_steps)
    
    # start training (!!)
    train(model, train_loader, val_loader, test_loader, loss_func, optimizer, scheduler, 
          num_epochs=num_epochs,
          start_epoch=0,
          val_every=len(train_loader),
          classification=True,
          runs_dir='runs'
    )    



    